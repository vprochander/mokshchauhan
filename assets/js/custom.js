     ...
     $(document).ready(function () {
        $('#mc_embed_signup').find('form').ajaxChimp({
            callback: function (resp) {
                ...
                var server_result_display = $("#mc_embed_signup").find(".info");

                if (resp && resp.result === 'success') {
                    server_result_display.empty().html('<div class="mb-0 mt-3 alert alert-danger  alert-dismissible">' + resp.msg + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                }else{
